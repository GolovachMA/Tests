...
Функционал: автоматическое формирование (через Помощник)  сверки взаиморасчетов
...
Сценарий: я формирую через Помощника создания сверку взаиморасчетов с контрагентом
* формирую сверку взаиморасчетов через Помощника создания
...
* проверка создания документа сверки
	И в таблице 'Список' я перехожу к строке
		...
		| 'Сверка взаиморасчетов (с фин.инструментами)' | 'Тест ООО'   | 'ТестКА1' |
	И в таблице "Список" я выбираю текущую строку
* проверка заполнения сверки взаиморасчетов вкладка Взаиморасчеты
	...
	Тогда таблица "ГруппировкиРасчеты" содержит строки по шаблону:
		| 'N' | 'Тип расчетов'          | 'Объект расчетов'                     | 'Партнер' | 'Договор' | 'Нач. остаток' | 'Нач. остаток (регл.)' | 'Приход'   | 'Расход' | 'Кон. остаток' | 'Кон. остаток (регл.)' |
		| '1' | 'Расчеты с поставщиком' | 'Возврат товаров поставщику * от *'  | 'ТестКА1' | 'Закупка' | ''             | ''                     | ''         | '324,00' | '-324,00'      | '-324,00'              |
		| '2' | 'Расчеты с поставщиком' | 'Приобретение товаров и услуг * от *' | 'ТестКА1' | 'Закупка' | ''             | ''                     | '1 080,00' | ''       | '1 080,00'     | '1 080,00'             |
		| '3' | 'Расчеты с поставщиком' | 'Списание безналичных ДС * от *'      | 'ТестКА1' | 'Закупка' | ''             | ''                     | ''         | '300,00' | '-300,00'      | '-300,00'              |	
